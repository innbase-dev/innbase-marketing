  ...
  if (typeof value === "boolean") {
    return (
      <span className={`v3-chip ${colClass}`}>
        <Icon name={value ? "check" : "x"} className={`icon ${value ? "yes" : "no"}`} />
      </span>
    );
  }
  return (
      ...
      <div className="wrap">
        ...
        <Reveal className="v3-toolbar reveal">
          <div className="v3-legend">
            <span>
              <span className="dot" style={{ background: "var(--brass-2)" }} />
              Boutique
            </span>
            <span>
              <span className="dot" style={{ background: "var(--teal-bright)" }} />
              Base
            </span>
            <span>
              <span className="dot" style={{ background: "var(--amber-bright)" }} />
              Growth
            </span>
          </div>
          ...
          {COMPARE_ROWS.map((row) => (
            ...
            >
              ...
              <span className="v3-ladder">
                ...
                <ValueChip value={row.values[1]} colClass="ba" />
                <span className="v3-arrow">→</span>
        ...
        <p className="v3-def-cap">
          A staff account is anyone with access to your workspace — owner, managers, front desk,
          bar &amp; housekeeping.
        </p>
      </div>
    </section>
