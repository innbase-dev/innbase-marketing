    ...
    <>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 20 }}>
        {data.kpis.map((kpi) => (
          <div className="mock-kpi" key={kpi.k}>
            ...
            <div className="v" style={{ color: kpi.color, fontSize: kpi.small ? 12 : undefined }}>
              {kpi.v}
            </div>
          </div>
        ))}
      ...
      {data.rows.map((row, i) => (
        <div className="mock-line" key={i}>
          <span
            className="mock-ava"
            style={{ background: row.ava, borderRadius: row.square ? 6 : undefined }}
          />
          <span>{row.label}</span>
          <span className="mock-chip" style={{ background: row.chipBg, color: row.chipColor }}>
            {row.chip}
          </span>
        </div>
      ))}
...
export default function DemoSection() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      const next = DEMO_ORDER[(DEMO_ORDER.indexOf(activeDemo) + 1) % DEMO_ORDER.length];
      activateDemo(next);
    }, 9000);
  ...
  return (
    <section className="sec-tight" id="demo" style={{ background: "var(--ink)" }}>
      <div className="wrap">
        <Reveal className="sec-head reveal">
          <span className="sec-eyebrow">Try it yourself</span>
          <h2 className="sec-h2">Tap around. It&apos;s the real thing.</h2>
          <p className="sec-sub">
            Every screen below is built on the same data model as your actual dashboard — nothing
            here is a mockup pretending to be a product.
          </p>
        </Reveal>
        ...
        <div className="mid-cta reveal">
          <p>
            <b>These are sample numbers.</b> Yours load in an afternoon — bank statements, POS,
            and stock counts included.
          </p>
