...
export default function sitemap() {
  return [
    ...
    // Thin topics (see BLOG.minPostsForTopicPage) are noindexed on the page
    // itself; leaving them out here too keeps the sitemap from asking Google
    // to crawl a page whose own meta tag then tells it not to index it.
    ...getCategories()
    ...
    // Each post's own last-edited date, not build time: a rebuild shouldn't
    // signal freshness for an article nobody actually touched.
    ...getPosts().map((post) => ({
